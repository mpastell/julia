...
StepRangeHiLo{T}(ref_hi::T, ref_lo::T, step_hi::T, step_lo::T, offset::Integer, len::Integer) =
...
function _linspace1{T}(::Type{T}, start, stop, len)
    ...
    if len <= 1
        ...
        # Ensure that first(r) returns start, and last(r) returns stop even for len=0
        step_hi = stop-start
        return StepRangeHiLo(start, zero(T), -step_hi, (start+step_hi) - stop, 1, len)
    end
...
convert{T<:AbstractFloat}(::Type{StepRangeHiLo{T}}, r::StepRangeHiLo{T}) = r
convert{T<:AbstractFloat}(::Type{StepRangeHiLo{T}}, r::StepRangeHiLo) =
...
function mul2{T<:Union{Float16,Float32,Float64}}(u_hi::T, u_lo::T, v::T)
...
_mul2{T<:Union{Float16,Float32,Float64}}(u_hi::T, u_lo::T, v::T) = mul2(u_hi, u_lo, v)
_mul2(u_hi, u_lo, v) = error("$u_hi::$(typeof(u_hi)), $u_lo::$(typeof(u_lo)), and $v::$(typeof(v)) cannot be promoted to a common AbstractFloat type")
